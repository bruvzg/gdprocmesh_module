...
#include "src/3rdparty/Simplify.h"

void GDProcSimplify::_bind_methods() {

	ClassDB::bind_method(D_METHOD("get_reduction_factor"), &GDProcSimplify::get_reduction);
	ClassDB::bind_method(D_METHOD("set_reduction_factor", "reduction_factor"), &GDProcSimplify::set_reduction);
	ADD_PROPERTY(PropertyInfo(Variant::REAL, "reduction_factor"), "set_reduction_factor", "get_reduction_factor");
}
	...
	if (updated) {
		...
		if (input_count > 0) {
			if (p_inputs[0].get_type() == Variant::ARRAY) {
				...
				// we keep default
				PoolVector<Vector3> source_v = source_array[ArrayMesh::ARRAY_VERTEX];
				int num_vertices = source_v.size();
				if (num_vertices > 0) {
					PoolVector<Vector3>::Read sv = source_v.read();
					Simplify::Vertex v;
					std::vector<int> vmap;
					...
					// into the triangle
					PoolVector<int> source_i = source_array[ArrayMesh::ARRAY_INDEX];
					int num_indices = source_i.size();
					if (num_indices) {
						PoolVector<int>::Read si = source_i.read();
						Simplify::Triangle t;

						PoolVector<Vector2> source_uv = source_array[ArrayMesh::ARRAY_TEX_UV];
						int num_uvs = source_uv.size();
						PoolVector<Vector2>::Read suv = source_uv.read();
						...
						PoolVector<Vector3> ns;
						PoolVector<real_t> ts;
						...
						// process our vertices
						{
							...
							PoolVector<Vector3>::Write vw = vs.write();
							PoolVector<Vector3>::Write nw = ns.write();
							PoolVector<real_t>::Write tw = ts.write();
						...
						// and our faces
						{
							...
							PoolVector<int>::Write iw = is.write();
							int di = 0;
...
String GDProcSimplify::get_connector_property_name(int p_slot) const {
	if (p_slot == 1) {
		return String("reduction_factor");
	}
