...
void GDProcExtrudeShape::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_shape_is_closed", "shape_is_closed"), &GDProcExtrudeShape::set_shape_is_closed);
	ADD_PROPERTY(PropertyInfo(Variant::BOOL, "shape_is_closed"), "set_shape_is_closed", "get_shape_is_closed");

	ClassDB::bind_method(D_METHOD("get_path_is_closed"), &GDProcExtrudeShape::get_path_is_closed);
	ClassDB::bind_method(D_METHOD("set_path_is_closed", "path_is_closed"), &GDProcExtrudeShape::set_path_is_closed);
	ADD_PROPERTY(PropertyInfo(Variant::BOOL, "path_is_closed"), "set_path_is_closed", "get_path_is_closed");

	ClassDB::bind_method(D_METHOD("get_path_follow"), &GDProcExtrudeShape::get_path_follow);
	ClassDB::bind_method(D_METHOD("set_path_follow", "path_follow"), &GDProcExtrudeShape::set_path_follow);
	ADD_PROPERTY(PropertyInfo(Variant::BOOL, "path_follow"), "set_path_follow", "get_path_follow");
}
...
bool GDProcExtrudeShape::update(bool p_inputs_updated, const Array &p_inputs) {
	...
	if (updated) {
		...
		int num_shape = 0;
		PoolVector<Vector3> shape;
		PoolVector<Vector3> shape_normals;
		...
		if ((num_shape >= (sic ? 3 : 2)) && (num_path >= (pic ? 3 : 2))) {
			...
			PoolVector<Vector3>::Read sr = shape.read();
			PoolVector<Vector3>::Read pr = path.read();
			...
			PoolVector<Vector3>::Read sn = shape_normals.read();
			...
			PoolVector<real_t>::Write tw = tangents.write();
...
String GDProcExtrudeShape::get_input_connector_name(int p_slot) const {
	if (p_slot == 0) {
		...
		return String("shape_is_closed");
	} else if (p_slot == 2) {
		return String("path");
	} else if (p_slot == 3) {
		return String("path_is_closed");
	} else if (p_slot == 4) {
		return String("path_follow");
	}
...
GDProcExtrudeShape::GDProcExtrudeShape() {
	// defaults
