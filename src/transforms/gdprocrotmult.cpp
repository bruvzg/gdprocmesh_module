...
bool GDProcRotMult::update(bool p_inputs_updated, const Array &p_inputs) {
	...
	if (updated) {
		int num_values_a = 0;
		PoolVector<Color> input_values_a;
		int num_values_b = 0;
		PoolVector<Color> input_values_b;
...

		{
			...
			PoolVector<Color>::Write w = values.write();
			PoolVector<Color>::Read a = input_values_a.read();
			PoolVector<Color>::Read b = input_values_b.read();
