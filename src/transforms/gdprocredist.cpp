...
void GDProcRedist::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_offset", "offset"), &GDProcRedist::set_offset);
	...
	ClassDB::bind_method(D_METHOD("set_strafe", "strafe"), &GDProcRedist::set_strafe);
	ADD_PROPERTY(PropertyInfo(Variant::REAL, "strafe"), "set_strafe", "get_strafe");

	ClassDB::bind_method(D_METHOD("get_is_closed"), &GDProcRedist::get_is_closed);
	ClassDB::bind_method(D_METHOD("set_is_closed", "is_closed"), &GDProcRedist::set_is_closed);
	...
	ClassDB::bind_method(D_METHOD("set_follow", "follow"), &GDProcRedist::set_follow);
...
bool GDProcRedist::update(bool p_inputs_updated, const Array &p_inputs) {
	...
	if (updated) {
		...
		int num_strafes = 0;
		PoolVector<real_t> strafes;
		int num_vectors = 0;
		PoolVector<Vector3> input_vectors;
		PoolVector<Color> input_rotations; // calculated, and we're abusing color here because its basically a vec4
		...
		if (input_count > 1) {
			if (p_inputs[1].get_type() == Variant::POOL_REAL_ARRAY) {
				...
				PoolVector<real_t> offsets = p_inputs[1];
				if (offsets.size() > 0) {
					offset = offsets[0];
				}
			}
		...
		if (num_vectors > 0) {
			...
			PoolVector<Color>::Write w = input_rotations.write();

			PoolVector<Vector3>::Read vec = input_vectors.read();
		...
		if (num_vectors > 0) {
			...
			PoolVector<Vector3>::Read vec = input_vectors.read();
			PoolVector<Color>::Read rot = input_rotations.read();
			PoolVector<real_t>::Read s = strafes.read();
