...
void GDProcBox::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_width", "width"), &GDProcBox::set_width);
	ADD_PROPERTY(PropertyInfo(Variant::REAL, "width"), "set_width", "get_width");

	ClassDB::bind_method(D_METHOD("get_height"), &GDProcBox::get_height);
	ClassDB::bind_method(D_METHOD("set_height", "height"), &GDProcBox::set_height);
	ADD_PROPERTY(PropertyInfo(Variant::REAL, "height"), "set_height", "get_height");

	ClassDB::bind_method(D_METHOD("get_depth"), &GDProcBox::get_depth);
	ClassDB::bind_method(D_METHOD("set_depth", "depth"), &GDProcBox::set_depth);
	...
	if (updated) {
		...
		if (input_count > 0) {
			if (p_inputs[0].get_type() == Variant::POOL_REAL_ARRAY) {
				PoolVector<real_t> input = p_inputs[0];
				if (input.size() > 0) {
					size.x = input[0];
				}
			}
		...
		if (input_count > 2) {
			if (p_inputs[2].get_type() == Variant::POOL_REAL_ARRAY) {
				PoolVector<real_t> input = p_inputs[2];
				if (input.size() > 0) {
					size.z = input[0];
				}
			}
		...
		{
			PoolVector<Vector3>::Write v = vertices.write();
		...
		{
			PoolVector<Vector3>::Write n = normals.write();
		...
		{
			PoolVector<real_t>::Write t = tangents.write();
			int i = 0;
		...
		{
			PoolVector<Vector2>::Write uv = uvs.write();
		...
		indices.resize(12 * 3);
		{
			PoolVector<int>::Write i = indices.write();
